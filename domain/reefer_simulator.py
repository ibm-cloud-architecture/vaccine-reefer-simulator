...
SIGMA_BASE = 1
products = { 'P01': {'d': 'covid-19','type': 1,'T': 4,'H': 40},
            'covid-19': {'d': 'covid-19','type': 1,'T': 4,'H': 40},
            'P02': {'d': 'covid-05','type': 2,'T': 6,'H': 60},
            'P03': {'d': 'ebola','type': 1,'T': 4,'H': 40},
            'P04': {'d': 'yellow-fever','type': 2,'T': 6,'H': 40},
            'P05': {'d': 'vid-20','type': 1,'T': 5,'H': 40}}
...
class ReeferSimulator:
    ...
    def generateTemperatureTuples(self,
                    ...
                    product_id: str = 'P01',
                    start_time: datetime.datetime = None):
        df = self.generateTemperatureRecords(cid, nb_records, product_id, start_time)
        return list(df.to_records(index=False))
